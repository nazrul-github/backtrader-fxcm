...
class MetafxcmData(DataBase.__class__):
    def __init__(self, name, bases, dct):
        '''Class has already been created ... register'''
        # Initialize the class
        super(MetafxcmData, self).__init__(name, bases, dct)

        # Register with the store
        fxcmstore.fxcmStore.DataCls = self


class fxcmData(with_metaclass(MetafxcmData, DataBase)):
    '''fxcm Data Feed.
    Params:
      - ``qcheck`` (default: ``0.5``)
    ...
    def setenvironment(self, env):
        ...
        belongs to'''
        super(fxcmData, self).setenvironment(env)
        env.addstore(self.o)

    def start(self):
        '''Starts the fxcm connection and gets the real contract and
        contractdetails if it exists'''
        super(fxcmData, self).start()
